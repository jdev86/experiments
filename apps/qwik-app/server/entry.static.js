...
var qwikReact = require("@builder.io/qwik-react");
...
const URL$1 = Url__default["default"].URL || whatwgUrl.URL;
...
const URL$1$1 = Url__default["default"].URL || whatwgUrl.URL;
...
const isQwikElement = (value) => isNode(value) && (value.nodeType === 1 || value.nodeType === 111);
const isVirtualElement = (value) => value.nodeType === 111;
const isModule = (module2) => isObject(module2) && module2[Symbol.toStringTag] === "Module";
let _platform = (() => {
  ...
  return {
    ...
    importSymbol(containerEl, url, symbolName) {
      ...
      return mod ? mod[symbolName] : function(t) {
...
const getDocument = (node) => typeof document != "undefined" ? document : node.nodeType === 9 ? node : node.ownerDocument;
...
const setKey = (el, key) => {
  key !== null && directSetAttribute(el, "q:key", key);
};
  ...
  for (; node; ) {
    if (isComment(node)) {
      if (node.data.startsWith("qv ")) {
        stack++;
      } else if (node.data === "/qv" && (stack--, stack === 0)) {
        return node;
      }
    }
...
const getRootNode = (node) => node == null ? null : isVirtualElement(node) ? node.open : node;
...
const findVirtual = (el) => {
  ...
  for (; node = node.previousSibling; ) {
    if (isComment(node)) {
      if (node.data === "/qv") {
        stack++;
      } else if (node.data.startsWith("qv ") && (stack--, stack === 0)) {
  ...
  if (node != null && typeof node != "boolean") {
    ...
    if (isJSXNode(node)) {
      return ((node2, invocationContext2) => {
        const key = node2.key != null ? String(node2.key) : null;
        ...
        return originalChildren != null ? then(processData$1(originalChildren, invocationContext2), (result) => (result !== void 0 && (children = isArray(result) ? result : [result]), new ProcessedJSXNodeImpl(textType, props, children, key))) : new ProcessedJSXNodeImpl(textType, props, children, key);
    ...
    return isPromise(node) ? node.then((node2) => processData$1(node2, invocationContext)) : node === SkipRender ? new ProcessedJSXNodeImpl(":skipRender", EMPTY_OBJ, EMPTY_ARRAY, null) : void logWarn("A unsupported value was passed to the JSX, skipping render. Value:", node);
  }
};
...
const smartUpdateChildren = (ctx, oldVnode, newVnode, mode, flags) => {
  ...
  if (ch.length === 1 && ch[0].$type$ === ":skipRender") {
    return;
  }
  const elm = oldVnode.$elm$;
  oldVnode.$children$ === CHILDREN_PLACEHOLDER && elm.nodeName === "HEAD" && (mode = "head", flags |= 2);
...
const updateChildren = (ctx, parentElm, oldCh, newCh, flags) => {
  ...
  for (; oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx; ) {
    if (oldStartVnode == null) {
      ...
      if (oldKeyToIdx === void 0 && (oldKeyToIdx = createKeyToOldIdx(oldCh, oldStartIdx, oldEndIdx)), idxInOld = oldKeyToIdx[newStartVnode.$key$], idxInOld === void 0) {
...
const getProps = (node) => {
  ...
  for (let i = 0; i < len; i++) {
    ...
    name.includes(":") || (props[name] = name === "class" ? parseDomClass(attr.value) : attr.value);
  }
  return props;
};
...
const isHeadChildren = (node) => {
  ...
  return type === 1 ? node.hasAttribute("q:head") : type === 111;
};
const isSlotTemplate = (node) => node.nodeName === "Q:TEMPLATE";
const isChildComponent = (node) => {
  ...
  return nodeName !== "Q:TEMPLATE" && (nodeName !== "HEAD" || node.hasAttribute("q:head"));
};
const patchVnode = (rctx, oldVnode, newVnode, flags) => {
  ...
  if (!isComponent) {
    ...
    if (isSvg && newVnode.$type$ === "foreignObject" && (flags &= -2, isSvg = false), isVirtual && "q:s" in props) {
      const currentComponent2 = rctx.$cmpCtx$;
      return currentComponent2.$slots$, void currentComponent2.$slots$.push(newVnode);
    }
    if (props[dangerouslySetInnerHTML] !== void 0) {
      return;
    }
...
const createElm = (rctx, vnode, flags) => {
  ...
  isVirtual ? elm = ((doc2) => {
    ...
    return new VirtualElementImpl(open, close);
  })(doc) : tag === "head" ? (elm = doc.head, flags |= 2, isHead = true) : (elm = createElement(doc, tag, isSvg), flags &= -3), vnode.$elm$ = elm, isSvg && tag === "foreignObject" && (isSvg = false, flags &= -2);
  const elCtx = getContext(elm);
  if (isComponent) {
    ...
    return setComponentProps$1(elCtx, rctx, props), setQId(rctx, elCtx), elCtx.$renderQrl$ = renderQRL, then(renderComponent(rctx, elCtx, flags), () => {
      ...
      children2.length === 1 && children2[0].$type$ === ":skipRender" && (children2 = children2[0].$children$);
  ...
  children.length === 1 && children[0].$type$ === ":skipRender" && (children = children[0].$children$);
...
const updateProperties = (elCtx, staticCtx, oldProps, newProps, isSvg) => {
  ...
  for (let key of keys) {
    ...
    key === "className" && (newProps.class = newValue, key = "class"), key === "class" && (newProps.class = newValue = serializeClass(newValue));
...
const setProperties = (rctx, elCtx, newProps, isSvg) => {
  ...
  for (let key of keys) {
    ...
    let newValue = newProps[key];
    if (key === "className" && (newProps.class = newValue, key = "class"), key === "class" && (newProps.class = newValue = serializeClass(newValue)), key === "ref") {
      newValue.current = elm;
      continue;
    }
    if (isOnProp(key)) {
...
const notifyWatch = (watch, containerState) => {
  watch.$flags$ & WatchFlagsIsDirty || (watch.$flags$ |= WatchFlagsIsDirty, containerState.$hostsRendering$ !== void 0 ? (containerState.$renderPromise$, containerState.$watchStaging$.add(watch)) : (containerState.$watchNext$.add(watch), scheduleFrame(containerState)));
};
const scheduleFrame = (containerState) => (containerState.$renderPromise$ === void 0 && (containerState.$renderPromise$ = getPlatform().nextTick(() => renderMarked(containerState))), containerState.$renderPromise$);
const _hW = () => {
...
const renderMarked = async (containerState) => {
  ...
  return staticCtx.$operations$.push(...staticCtx.$postOperations$), staticCtx.$operations$.length === 0 ? void postRendering(containerState, staticCtx) : getPlatform().raf(() => {
    (({ $static$: ctx2 }) => {
...
const getFlags = (el) => {
  let flags = 0;
  return el && (el.namespaceURI === SVG_NS && (flags |= 1), el.tagName === "HEAD" && (flags |= 2)), flags;
};
const postRendering = async (containerState, ctx) => {
  await executeWatchesAfter(containerState, (watch, stage) => (watch.$flags$ & WatchFlagsIsEffect) != 0 && (!stage || ctx.$hostElements$.has(watch.$el$))), containerState.$hostsStaging$.forEach((el) => {
...
const sortWatches = (watches) => {
  watches.sort((a2, b) => a2.$el$ === b.$el$ ? a2.$index$ < b.$index$ ? -1 : 1 : (2 & a2.$el$.compareDocumentPosition(getRootNode(b.$el$))) != 0 ? 1 : -1);
};
...
const createSubscriptionManager = (containerState) => {
  ...
  const trackSubToObj = (subscriber, map2) => {
    ...
    set || subsToObjs.set(subscriber, set = /* @__PURE__ */ new Set()), set.add(map2);
  };
  const manager = {
        ...
        map2.forEach((_, key) => {
          ...
          $addSub$(subscriber, key) {
            if (key == null) {
              ...
              let sub = map2.get(subscriber);
              sub === void 0 && map2.set(subscriber, sub = /* @__PURE__ */ new Set()), sub && sub.add(key);
            }
            trackSubToObj(subscriber, map2);
          },
          $notifySubs$(key) {
            map2.forEach((value, subscriber) => {
              value !== null && key && !value.has(key) || ((subscriber2, containerState2) => {
                isQwikElement(subscriber2) ? ((hostElement, containerState3) => {
                  ...
                  if (ctx.$renderQrl$, !ctx.$dirty$) {
                    if (ctx.$dirty$ = true, containerState3.$hostsRendering$ !== void 0) {
...
const reviveNestedObjects = (obj, getObject, parser) => {
  if (!parser.fill(obj) && obj && typeof obj == "object") {
    if (isArray(obj)) {
      for (let i = 0; i < obj.length; i++) {
        const value = obj[i];
        typeof value == "string" ? obj[i] = getObject(value) : reviveNestedObjects(value, getObject, parser);
      }
    } else if (isSerializableObject(obj)) {
      for (const key of Object.keys(obj)) {
        const value = obj[key];
        typeof value == "string" ? obj[key] = getObject(value) : reviveNestedObjects(value, getObject, parser);
      }
    }
  }
};
...
const useWatchQrl = (qrl, opts) => {
  ...
  const elCtx = getContext(el);
  set(true), qrl.$resolveLazy$(containerState.$containerEl$), elCtx.$watches$ || (elCtx.$watches$ = []), elCtx.$watches$.push(watch), waitAndRun(ctx, () => runSubscriber(watch, containerState, ctx.$renderCtx$));
};
const useClientEffectQrl = (qrl, opts) => {
  ...
  const elCtx = getContext(el);
  const containerState = ctx.$renderCtx$.$static$.$containerState$;
  set(true), elCtx.$watches$ || (elCtx.$watches$ = []), elCtx.$watches$.push(watch), useRunWatch(watch, eagerness), qrl.$resolveLazy$(containerState.$containerEl$), notifyWatch(watch, containerState);
};
...
const useRunWatch = (watch, eagerness) => {
  eagerness === "load" ? useOn("qinit", getWatchHandlerQrl(watch)) : eagerness === "visible" && useOn("qvisible", getWatchHandlerQrl(watch));
};
...
const useResourceQrl = (qrl, opts) => {
  ...
  const elCtx = getContext(el);
  return runResource(watch, containerState, previousWait), elCtx.$watches$ || (elCtx.$watches$ = []), elCtx.$watches$.push(watch), set(resource), resource;
};
const Resource = (props) => {
  if (props.onRejected && (props.value.promise.catch(() => {
  }), props.value.state === "rejected")) {
...
const isResourceReturn = (obj) => isObject(obj) && obj.__brand === "resource";
...
const ResourceSerializer = {
  ...
  serialize: (obj, getObjId) => ((resource, getObjId2) => {
    ...
    return state === "resolved" ? `0 ${getObjId2(resource.resolved)}` : state === "pending" ? "1" : `2 ${getObjId2(resource.error)}`;
  })(obj, getObjId),
  prepare: (data) => ((data2) => {
    const [first, id] = data2.split(" ");
    const result = _createResourceReturn(void 0);
    return result.promise = Promise.resolve(), first === "0" ? (result.state = "resolved", result.resolved = id) : first === "1" ? (result.state = "pending", result.promise = new Promise(() => {
    })) : first === "2" && (result.state = "rejected", result.error = id), result;
  })(data),
...
class ReadWriteProxyHandler {
  ...
  get(target, prop) {
    ...
    const recursive = (1 & this.$flags$) != 0;
    const immutable = (2 & this.$flags$) != 0;
  ...
  set(target, prop, newValue) {
    ...
    const unwrappedNewValue = (1 & this.$flags$) != 0 ? unwrapProxy(newValue) : newValue;
...
const noSerialize = (input) => (input != null && noSerializeSet.add(input), input);
...
const resumeIfNeeded = (containerEl) => {
  directGetAttribute(containerEl, "q:container") === "paused" && (((containerEl2) => {
    ...
    const getObject = (id) => ((id2, elements2, objs, containerState2) => {
      if (typeof id2 == "string" && id2.length, id2.startsWith("#")) {
    ...
    var el;
    directSetAttribute(containerEl2, "q:container", "resumed"), (el = containerEl2) && typeof CustomEvent == "function" && el.dispatchEvent(new CustomEvent("qresume", {
      detail: void 0,
      bubbles: true,
      composed: true
    }));
...
const appendQwikDevTools = (containerEl) => {
  containerEl.qwik = {
    pause: () => (async (elmOrDoc, defaultParentJSON) => {
      ...
      if (directGetAttribute(containerEl2, "q:container") === "paused") {
...
const logError = (message, ...optionalParams) => {
  ...
  return typeof globalThis._handleError == "function" && message instanceof Error ? globalThis._handleError(message, optionalParams) : console.error("%cQWIK ERROR", "", err.message, ...printParams(optionalParams), err.stack), err;
};
...
const isQrl = (value) => typeof value == "function" && typeof value.getSymbol == "function";
const createQRL = (chunk, symbol, symbolRef, symbolFn, capture, captureRef, refSymbol) => {
  ...
  const invokeFn = (currentCtx, beforeFn) => (...args) => {
    ...
    return then(fn, (fn2) => {
      if (isFunction(fn2)) {
        ...
        const context = __spreadProps(__spreadValues({}, createInvokationContext(currentCtx)), {
          $qrl$: QRL
        });
  ...
  const createInvokationContext = (invoke2) => invoke2 == null ? newInvokeContext() : isArray(invoke2) ? newInvokeContextFromTuple(invoke2) : invoke2;
...
const emitUsedSymbol = (symbol, element) => {
  typeof document != "object" || document.dispatchEvent(new CustomEvent("qsymbol", {
    bubbles: false,
    detail: {
      symbol,
      element,
...
const inlinedQrl = (symbol, symbolName, lexicalScopeCapture = EMPTY_ARRAY) => createQRL("/inlinedQRL", symbolName, symbol, null, null, lexicalScopeCapture, null);
const stringifyQRL = (qrl, opts = {}) => {
  ...
  const parts = [chunk];
  symbol && symbol !== "default" && parts.push("#", symbol);
...
const serializeQRLs = (existingQRLs, elCtx) => {
  var value;
  (function(value2) {
    return value2 && typeof value2.nodeType == "number";
...
const parseQRL = (qrl, containerEl) => {
  ...
  chunk === "/runtimeQRL" && logError(codeToText(2), qrl);
...
const addToArray = (array, obj) => {
  ...
  return index2 === -1 ? (array.push(obj), array.length - 1) : index2;
};
...
const componentQrl = (onRenderQrl) => {
  function QwikComponent(props, key) {
    ...
    return jsx(Virtual, __spreadValues({
      "q:renderFn": onRenderQrl
    }, props), hash + ":" + (key || ""));
  }
  return QwikComponent[SERIALIZABLE_STATE] = [onRenderQrl], QwikComponent;
};
const isQwikComponent = (component) => typeof component == "function" && component[SERIALIZABLE_STATE] !== void 0;
const Slot = (props) => {
  var _a2;
...
const scopeStylesheet = (css, scopeId) => {
  ...
  for (; idx < end; ) {
    ...
    for (let i = 0; i < arcs.length; i++) {
      ...
      if ((expectLastCh === lastCh || expectLastCh === ANY || expectLastCh === IDENT && isIdent(lastCh) || expectLastCh === WHITESPACE && isWhiteSpace(lastCh)) && (expectCh === ch || expectCh === ANY || expectCh === IDENT && isIdent(ch) || expectCh === NOT_IDENT && !isIdent(ch) && ch !== DOT || expectCh === WHITESPACE && isWhiteSpace(ch)) && (arc.length == 3 || lookAhead(arc))) {
...
var Header = /* @__PURE__ */ componentQrl(inlinedQrl(() => {
...
var layout = /* @__PURE__ */ componentQrl(inlinedQrl(() => {
...
var Layout_ = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  "default": layout
}, Symbol.toStringTag, { value: "Module" }));
const isServer = true;
...
const resolveHead = (endpoint, routeLocation, contentModules) => {
  ...
  const headProps = __spreadValues({
...
const loadClientData = async (href) => {
  const { cacheModules: cacheModules2 } = await Promise.resolve().then(function() {
    return _qwikCityPlan;
  });
...
const QwikCity = /* @__PURE__ */ componentQrl(inlinedQrl(() => {
  ...
  useWatchQrl(inlinedQrl(async ({ track }) => {
    ...
    const { routes: routes2, menus: menus2, cacheModules: cacheModules2 } = await Promise.resolve().then(function() {
      return _qwikCityPlan;
    });
    const path2 = track(routeNavigate2, "path");
    const url2 = new URL(path2, routeLocation2.href);
    ...
    if (loadedRoute) {
      ...
      routeLocation2.params = __spreadValues({}, params);
...
const Link = /* @__PURE__ */ componentQrl(inlinedQrl((props) => {
  ...
  const linkProps = __spreadValues({}, props);
  ...
  return /* @__PURE__ */ jsx("a", __spreadProps(__spreadValues({}, linkProps), {
    onClick$: inlinedQrl(() => {
...
var index$3 = /* @__PURE__ */ componentQrl(inlinedQrl(() => {
...
var index$2 = /* @__PURE__ */ componentQrl(inlinedQrl(() => {
...
var Flower = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  "default": index$2,
  head: head$2
}, Symbol.toStringTag, { value: "Module" }));
const MUIButton = qwikReact.qwikifyQrl(inlinedQrl(() => {
...
var index$1 = /* @__PURE__ */ componentQrl(inlinedQrl(() => {
  return /* @__PURE__ */ jsx("div", {
    children: [
      /* @__PURE__ */ jsx("h1", {
        children: "The component below is a React MUI button:"
      }),
      /* @__PURE__ */ jsx(MUIButton, {})
    ]
  });
}, "s_i4frY0Pt5lk"));
const head$1 = {
...
const WeatherComponent = /* @__PURE__ */ componentQrl(inlinedQrl(() => {
  const github = useStore({
    org: "BuilderIO"
  });
  const reposResource = useResourceQrl(inlinedQrl(({ track, cleanup }) => {
    const [github2] = useLexicalScope();
    track(github2, "org");
    const controller = new AbortController();
    cleanup(() => controller.abort());
    return getRepositories(github2.org, controller);
  }, "s_ZQyMc6pgyTk", [
    github
  ]));
  console.log("Render");
    ...
    children: [
        ...
        children: [
          ...
          /* @__PURE__ */ jsx("input", {
            ...
            onKeyUp$: inlinedQrl((ev) => {
              const [github2] = useLexicalScope();
              return github2.org = ev.target.value;
            }, "s_nm91j5JZMoI", [
              github
            ])
          })
      ...
      /* @__PURE__ */ jsx("div", {
        children: /* @__PURE__ */ jsx(Resource, {
          value: reposResource,
          onPending: () => /* @__PURE__ */ jsx(Fragment, {
            children: "Loading..."
          }),
          onRejected: (error2) => /* @__PURE__ */ jsx(Fragment, {
            children: [
              "Error: ",
              error2.message
            ]
          }),
          onResolved: (repos) => /* @__PURE__ */ jsx("ul", {
            children: repos.map((repo) => /* @__PURE__ */ jsx("li", {
              children: /* @__PURE__ */ jsx("a", {
...
async function getRepositories(username, controller) {
  ...
  return Array.isArray(json) ? json.map((repo) => repo.name) : Promise.reject(json);
}
var index = /* @__PURE__ */ componentQrl(inlinedQrl(() => {
  return /* @__PURE__ */ jsx(WeatherComponent, {});
}, "s_HoYXtYWcYPQ"));
const head = {
  title: "Qwik Weather"
};
var Weather = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  "default": index,
  head
}, Symbol.toStringTag, { value: "Module" }));
...
const routes = [
  [/^\/$/, [Layout, () => Index], void 0, "/", ["q-9e6ea017.js", "q-bf0e0b15.js"]],
  [/^\/flower\/?$/, [Layout, () => Flower], void 0, "/flower", ["q-9e6ea017.js", "q-1d501809.js"]],
  [/^\/react\/?$/, [Layout, () => React], void 0, "/react", ["q-9e6ea017.js", "q-fd23dcb3.js"]],
  [/^\/weather\/?$/, [Layout, () => Weather], void 0, "/weather", ["q-9e6ea017.js", "q-83312463.js"]]
];
const menus = [];
const trailingSlash = false;
const basePathname = "/";
const cacheModules = true;
var _qwikCityPlan = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  routes,
  menus,
  trailingSlash,
...
function normalizePath(path$1) {
  ...
  const isExtendedLengthPath = /^\\\\\?\\/.test(path$1);
  const hasNonAscii = /[^\u0000-\u0080]+/.test(path$1);
  if (isExtendedLengthPath || hasNonAscii) {
    return path$1;
  }
  path$1 = path$1.replace(/\\/g, "/");
  if (path$1.endsWith("/")) {
    path$1 = path$1.slice(0, path$1.length - 1);
  }
  return path$1;
}
async function createNodeMainProcess(opts) {
  const currentFile = Url.fileURLToPath(typeof document === "undefined" ? new (require("url")).URL("file:" + __filename).href : document.currentScript && document.currentScript.src || new URL("entry.static.js", document.baseURI).href);
  const ssgWorkers = [];
  const sitemapBuffer = [];
  ...
  let maxWorkers = os.cpus().length - 1;
  ...
  if (sitemapOutFile !== null) {
    ...
    if (!path.isAbsolute(sitemapOutFile)) {
      sitemapOutFile = path.join(outDir, sitemapOutFile);
    }
  }
  const createWorker = () => {
    let terminateResolve = null;
    const mainTasks = /* @__PURE__ */ new Map();
    const nodeWorker = new worker_threads.Worker(currentFile);
    const ssgWorker = {
      activeTasks: 0,
      totalTasks: 0,
      render: (staticRoute) => {
  ...
  const render2 = async (staticRoute) => {
    const ssgWorker = getNextWorker();
    ...
    if (sitemapOutFile && result.ok) {
      ...
      if (sitemapBuffer.length > 50) {
        ...
        sitemapPromise = fs__default["default"].promises.appendFile(sitemapOutFile, siteMapUrls);
      }
    }
    return result;
  };
  const close = async () => {
    ...
    if (sitemapOutFile) {
      ...
      promises.push(fs__default["default"].promises.appendFile(sitemapOutFile, sitemapBuffer.join("\n")));
      sitemapBuffer.length = 0;
    }
  ...
  if (sitemapOutFile) {
    ...
    await fs__default["default"].promises.writeFile(sitemapOutFile, `<?xml version="1.0" encoding="UTF-8"?>
...
function ssgWorkerCompare(a2, b) {
  if (a2.activeTasks < b.activeTasks) {
    return -1;
  }
  if (a2.activeTasks > b.activeTasks) {
    return 1;
  }
  return a2.totalTasks < b.totalTasks ? -1 : 1;
}
async function createNodeWorkerProcess(onMessage) {
  var _a2;
  (_a2 = worker_threads.parentPort) == null ? void 0 : _a2.on("message", async (msg) => {
    var _a3;
    (_a3 = worker_threads.parentPort) == null ? void 0 : _a3.postMessage(await onMessage(msg));
  });
}
async function createNodeSystem(opts) {
  opts = __spreadValues({}, opts);
  patchGlobalFetch();
  const createWriteStream = (filePath) => {
    return fs__default["default"].createWriteStream(filePath, {
      flags: "w"
    });
  };
...
var ensureDir = async (filePath) => {
  try {
    await fs__default["default"].promises.mkdir(path.dirname(filePath), { recursive: true });
  } catch (e) {
  }
};
    ...
    try {
      ...
      const next = () => {
        ...
        if (!isCompleted && isRoutesLoaded && queue.length === 0 && active.size === 0) {
          ...
          log.info(`Generated: ${generatorResults.rendered} page${generatorResults.rendered === 1 ? "" : "s"}`);
          log.info(`Duration: ${msToString(generatorResults.duration)}`);
          if (generatorResults.rendered > 0) {
            log.info(`Average: ${msToString(generatorResults.duration / generatorResults.rendered)} per page`);
          }
      ...
      const render2 = async (staticRoute) => {
        try {
          ...
          const result = await main.render(__spreadValues({ type: "render" }, staticRoute));
...
function errorHandler(requestCtx, e) {
  ...
  return requestCtx.response(status, headers, async (stream) => {
    stream.write(html);
  }, e);
}
function errorResponse(requestCtx, errorResponse2) {
  const html = minimalHtmlResponse(errorResponse2.status, errorResponse2.message, errorResponse2.stack);
  const headers = createHeaders();
  headers.set("Content-Type", "text/html; charset=utf-8");
  return requestCtx.response(errorResponse2.status, headers, async (stream) => {
    stream.write(html);
  }, errorResponse2);
}
...
async function loadUserResponse(requestCtx, params, routeModules, platform, trailingSlash3, basePathname3 = "/") {
  ...
  if (isPageModule && pathname !== basePathname3) {
    if (trailingSlash3) {
      ...
      if (pathname.endsWith("/")) {
        throw new RedirectResponse(pathname.slice(0, pathname.length - 1) + url.search, 307);
      }
    }
  }
...
function pageHandler(requestCtx, userResponse, render2, opts, routeBundleNames) {
  ...
  return response(isPageData ? 200 : status, headers, async (stream) => {
    const result = await render2(__spreadValues({
...
async function requestHandler(requestCtx, render2, platform, opts) {
  try {
    ...
    if (loadedRoute) {
      ...
      const userResponse = await loadUserResponse(requestCtx, params, mods, platform, trailingSlash, basePathname);
...
async function workerThread(sys, render2) {
  const opts = sys.getOptions();
    ...
    switch (msg.type) {
      case "render": {
        return new Promise((resolve) => {
          workerRender(sys, render2, opts, msg, pendingPromises, resolve);
        });
      }
      case "close": {
...
async function workerRender(sys, render2, opts, staticRoute, pendingPromises, callback) {
  ...
  try {
    ...
    const promise = requestHandler(requestCtx, render2, {}, __spreadValues(__spreadValues({}, opts), staticRoute)).then((rsp) => {
      if (rsp == null) {
        callback(result);
      }
    }).catch((e) => {
...
async function staticGenerate(sys, render2) {
  if (sys.isMainThread()) {
    await mainThread(sys);
  } else {
    await workerThread(sys, render2);
  }
}
async function qwikCityGenerate(render2, opts) {
  try {
    const nodeSys = await createNodeSystem(opts);
    await staticGenerate(nodeSys, render2);
  } catch (e) {
    console.error(e);
  }
}
const manifest = { "symbols": { "s_nm91j5JZMoI": { "origin": "components/weather/index.tsx", "displayName": "WeatherComponent_component_div_span_input_onKeyUp", "canonicalFilename": "s_nm91j5jzmoi", "hash": "nm91j5JZMoI", "ctxKind": "event", "ctxName": "onKeyUp$", "captures": true, "parent": "s_mJK9x0YPP3U" }, "s_dznIGAlrcag": { "origin": "routes/flower/index.tsx", "displayName": "flower_component__Fragment_input_onInput", "canonicalFilename": "s_dznigalrcag", "hash": "dznIGAlrcag", "ctxKind": "event", "ctxName": "onInput$", "captures": true, "parent": "s_OW4nu0I1yZ8" }, "s_x04JC5xeP1U": { "origin": "../node_modules/@builder.io/qwik-react/lib/index.qwik.mjs", "displayName": "qwikifyQrl_component_useWatch", "canonicalFilename": "s_x04jc5xep1u", "hash": "x04JC5xeP1U", "ctxKind": "function", "ctxName": "useWatch$", "captures": true, "parent": "s_zH94hIe0Ick" }, "s_V0Y6u0VD1eY": { "origin": "routes/flower/index.tsx", "displayName": "flower_component_useClientEffect", "canonicalFilename": "s_v0y6u0vd1ey", "hash": "V0Y6u0VD1eY", "ctxKind": "function", "ctxName": "useClientEffect$", "captures": true, "parent": "s_OW4nu0I1yZ8" }, "s_3sccYCDd1Z0": { "origin": "root.tsx", "displayName": "root_component", "canonicalFilename": "s_3sccycdd1z0", "hash": "3sccYCDd1Z0", "ctxKind": "function", "ctxName": "component$", "captures": false, "parent": null }, "s_HoYXtYWcYPQ": { "origin": "routes/weather/index.tsx", "displayName": "weather_component", "canonicalFilename": "s_hoyxtywcypq", "hash": "HoYXtYWcYPQ", "ctxKind": "function", "ctxName": "component$", "captures": false, "parent": null }, "s_OW4nu0I1yZ8": { "origin": "routes/flower/index.tsx", "displayName": "flower_component", "canonicalFilename": "s_ow4nu0i1yz8", "hash": "OW4nu0I1yZ8", "ctxKind": "function", "ctxName": "component$", "captures": false, "parent": null }, "s_VkLNXphUh5s": { "origin": "routes/layout.tsx", "displayName": "layout_component", "canonicalFilename": "s_vklnxphuh5s", "hash": "VkLNXphUh5s", "ctxKind": "function", "ctxName": "component$", "captures": false, "parent": null }, "s_ceU05TscGYE": { "origin": "components/header/header.tsx", "displayName": "header_component", "canonicalFilename": "s_ceu05tscgye", "hash": "ceU05TscGYE", "ctxKind": "function", "ctxName": "component$", "captures": false, "parent": null }, "s_i4frY0Pt5lk": { "origin": "routes/react/index.tsx", "displayName": "react_component", "canonicalFilename": "s_i4fry0pt5lk", "hash": "i4frY0Pt5lk", "ctxKind": "function", "ctxName": "component$", "captures": false, "parent": null }, "s_mJK9x0YPP3U": { "origin": "components/weather/index.tsx", "displayName": "WeatherComponent_component", "canonicalFilename": "s_mjk9x0ypp3u", "hash": "mJK9x0YPP3U", "ctxKind": "function", "ctxName": "component$", "captures": false, "parent": null }, "s_xYL1qOwPyDI": { "origin": "routes/index.tsx", "displayName": "routes_component", "canonicalFilename": "s_xyl1qowpydi", "hash": "xYL1qOwPyDI", "ctxKind": "function", "ctxName": "component$", "captures": false, "parent": null }, "s_zH94hIe0Ick": { "origin": "../node_modules/@builder.io/qwik-react/lib/index.qwik.mjs", "displayName": "qwikifyQrl_component", "canonicalFilename": "s_zh94hie0ick", "hash": "zH94hIe0Ick", "ctxKind": "function", "ctxName": "component$", "captures": true, "parent": null }, "s_zrbrqoaqXSY": { "origin": "components/router-head/router-head.tsx", "displayName": "RouterHead_component", "canonicalFilename": "s_zrbrqoaqxsy", "hash": "zrbrqoaqXSY", "ctxKind": "function", "ctxName": "component$", "captures": false, "parent": null }, "s_N39ca0w8E8Y": { "origin": "components/header/header.tsx", "displayName": "header_component_useStylesScoped", "canonicalFilename": "s_n39ca0w8e8y", "hash": "N39ca0w8E8Y", "ctxKind": "function", "ctxName": "useStylesScoped$", "captures": false, "parent": "s_ceU05TscGYE" }, "s_p4UiTwsJc7c": { "origin": "routes/flower/index.tsx", "displayName": "flower_component_useStylesScoped", "canonicalFilename": "s_p4uitwsjc7c", "hash": "p4UiTwsJc7c", "ctxKind": "function", "ctxName": "useStylesScoped$", "captures": false, "parent": "s_OW4nu0I1yZ8" }, "s_r35YTGxHeL0": { "origin": "integrations/react/button.tsx", "displayName": "MUIButton_qwikify", "canonicalFilename": "s_r35ytgxhel0", "hash": "r35YTGxHeL0", "ctxKind": "function", "ctxName": "qwikify$", "captures": false, "parent": null }, "s_ZQyMc6pgyTk": { "origin": "components/weather/index.tsx", "displayName": "WeatherComponent_component_reposResource_useResource", "canonicalFilename": "s_zqymc6pgytk", "hash": "ZQyMc6pgyTk", "ctxKind": "function", "ctxName": "useResource$", "captures": true, "parent": "s_mJK9x0YPP3U" } }, "mapping": { "s_nm91j5JZMoI": "q-7fec964f.js", "s_dznIGAlrcag": "q-cb51c4f2.js", "s_x04JC5xeP1U": "q-b9fd99b5.js", "s_V0Y6u0VD1eY": "q-cb51c4f2.js", "s_3sccYCDd1Z0": "q-fccc4c26.js", "s_HoYXtYWcYPQ": "q-de034b6e.js", "s_OW4nu0I1yZ8": "q-cb51c4f2.js", "s_VkLNXphUh5s": "q-ccb17771.js", "s_ceU05TscGYE": "q-9ee91a9c.js", "s_i4frY0Pt5lk": "q-0468e5e4.js", "s_mJK9x0YPP3U": "q-7fec964f.js", "s_xYL1qOwPyDI": "q-24f7db0f.js", "s_zH94hIe0Ick": "q-b9fd99b5.js", "s_zrbrqoaqXSY": "q-c6f81dd2.js", "s_N39ca0w8E8Y": "q-9ee91a9c.js", "s_p4UiTwsJc7c": "q-cb51c4f2.js", "s_r35YTGxHeL0": "q-e9d9d4cc.js", "s_ZQyMc6pgyTk": "q-7fec964f.js" }, "bundles": { "q-0468e5e4.js": { "size": 579, "imports": ["q-2010d20b.js"], "dynamicImports": ["q-b9fd99b5.js", "q-e9d9d4cc.js"], "origins": ["node_modules/@builder.io/qwik-react/lib/index.qwik.mjs", "src/entry_react.js", "src/integrations/react/button.js", "src/s_i4fry0pt5lk.js"], "symbols": ["s_i4frY0Pt5lk"] }, "q-0b915655.js": { "size": 25292, "origins": ["\0/Users/justin_mills/Documents/playground/experiments/apps/qwik-app/node_modules/hoist-non-react-statics/node_modules/react-is/cjs/react-is.production.min.js?commonjs-exports", "\0/Users/justin_mills/Documents/playground/experiments/apps/qwik-app/node_modules/hoist-non-react-statics/node_modules/react-is/index.js?commonjs-module", "\0/Users/justin_mills/Documents/playground/experiments/apps/qwik-app/node_modules/react/cjs/react-jsx-runtime.production.min.js?commonjs-exports", "\0/Users/justin_mills/Documents/playground/experiments/apps/qwik-app/node_modules/react/cjs/react.production.min.js?commonjs-exports", "\0/Users/justin_mills/Documents/playground/experiments/apps/qwik-app/node_modules/react/index.js?commonjs-module", "\0/Users/justin_mills/Documents/playground/experiments/apps/qwik-app/node_modules/react/jsx-runtime.js?commonjs-module", "node_modules/@emotion/cache/dist/emotion-cache.browser.esm.js", "node_modules/@emotion/hash/dist/emotion-hash.esm.js", "node_modules/@emotion/memoize/dist/emotion-memoize.esm.js", "node_modules/@emotion/react/dist/emotion-element-cbed451f.browser.esm.js", "node_modules/@emotion/serialize/dist/emotion-serialize.browser.esm.js", "node_modules/@emotion/sheet/dist/emotion-sheet.browser.esm.js", "node_modules/@emotion/unitless/dist/emotion-unitless.esm.js", "node_modules/@emotion/utils/dist/emotion-utils.browser.esm.js", "node_modules/hoist-non-react-statics/dist/hoist-non-react-statics.cjs.js", "node_modules/hoist-non-react-statics/node_modules/react-is/cjs/react-is.production.min.js", "node_modules/hoist-non-react-statics/node_modules/react-is/index.js", "node_modules/react/cjs/react-jsx-runtime.production.min.js", "node_modules/react/cjs/react.production.min.js", "node_modules/react/index.js", "node_modules/react/jsx-runtime.js", "node_modules/stylis/src/Enum.js", "node_modules/stylis/src/Middleware.js", "node_modules/stylis/src/Parser.js", "node_modules/stylis/src/Prefixer.js", "node_modules/stylis/src/Serializer.js", "node_modules/stylis/src/Tokenizer.js", "node_modules/stylis/src/Utility.js"] }, "q-143c7194.js": { "size": 2198, "origins": ["../../node_modules/@builder.io/qwik-city/service-worker.mjs", "src/routes/service-worker.js"] }, "q-1d501809.js": { "size": 196, "imports": ["q-2010d20b.js"], "dynamicImports": ["q-cb51c4f2.js"], "origins": ["src/routes/flower/index.js"] }, "q-2010d20b.js": { "size": 37661, "dynamicImports": ["q-fccc4c26.js"], "origins": ["node_modules/@builder.io/qwik/core.min.mjs", "src/global.css", "src/root.js", "vite/preload-helper"] }, "q-24f7db0f.js": { "size": 2698, "imports": ["q-2010d20b.js", "q-fccc4c26.js"], "origins": ["src/entry_routes.js", "src/s_xyl1qowpydi.js"], "symbols": ["s_xYL1qOwPyDI"] }, "q-7fec964f.js": { "size": 1475, "imports": ["q-2010d20b.js"], "origins": ["src/components/weather/index.js", "src/entry_WeatherComponent.js", "src/s_mjk9x0ypp3u.js", "src/s_nm91j5jzmoi.js", "src/s_zqymc6pgytk.js"], "symbols": ["s_mJK9x0YPP3U", "s_nm91j5JZMoI", "s_ZQyMc6pgyTk"] }, "q-83312463.js": { "size": 197, "imports": ["q-2010d20b.js"], "dynamicImports": ["q-de034b6e.js"], "origins": ["src/routes/weather/index.js"] }, "q-9949742d.js": { "size": 580, "imports": ["q-2010d20b.js"], "dynamicImports": ["q-1d501809.js", "q-83312463.js", "q-9e6ea017.js", "q-a93ff783.js", "q-bf0e0b15.js", "q-fd23dcb3.js"], "origins": ["@qwik-city-plan"] }, "q-9e6ea017.js": { "size": 156, "imports": ["q-2010d20b.js"], "dynamicImports": ["q-ccb17771.js"], "origins": ["src/routes/layout.js"] }, "q-9ee91a9c.js": { "size": 4164, "imports": ["q-2010d20b.js"], "origins": ["src/components/header/header.css?used&inline", "src/components/icons/qwik.js", "src/entry_header.js", "src/s_ceu05tscgye.js", "src/s_n39ca0w8e8y.js"], "symbols": ["s_ceU05TscGYE", "s_N39ca0w8E8Y"] }, "q-a4cc4d7c.js": { "size": 135601, "imports": ["q-0b915655.js"], "origins": ["\0/Users/justin_mills/Documents/playground/experiments/apps/qwik-app/node_modules/react-dom/cjs/react-dom.production.min.js?commonjs-exports", "\0/Users/justin_mills/Documents/playground/experiments/apps/qwik-app/node_modules/react-dom/client.js?commonjs-exports", "\0/Users/justin_mills/Documents/playground/experiments/apps/qwik-app/node_modules/react-dom/index.js?commonjs-module", "\0/Users/justin_mills/Documents/playground/experiments/apps/qwik-app/node_modules/scheduler/cjs/scheduler.production.min.js?commonjs-exports", "\0/Users/justin_mills/Documents/playground/experiments/apps/qwik-app/node_modules/scheduler/index.js?commonjs-module", "node_modules/@builder.io/qwik-react/lib/client.1abdbc9a.js", "node_modules/@emotion/react/jsx-runtime/dist/emotion-react-jsx-runtime.browser.esm.js", "node_modules/react-dom/cjs/react-dom.production.min.js", "node_modules/react-dom/client.js", "node_modules/react-dom/index.js", "node_modules/scheduler/cjs/scheduler.production.min.js", "node_modules/scheduler/index.js"] }, "q-a93ff783.js": { "size": 128, "imports": ["q-2010d20b.js"], "dynamicImports": ["q-143c7194.js"], "origins": ["@qwik-city-entries"] }, "q-b9fd99b5.js": { "size": 1140, "imports": ["q-0468e5e4.js", "q-2010d20b.js"], "dynamicImports": ["q-a4cc4d7c.js"], "origins": ["src/entry_qwikifyQrl.js", "src/s_x04jc5xep1u.js", "src/s_zh94hie0ick.js"], "symbols": ["s_x04JC5xeP1U", "s_zH94hIe0Ick"] }, "q-bf0e0b15.js": { "size": 200, "imports": ["q-2010d20b.js"], "dynamicImports": ["q-24f7db0f.js"], "origins": ["src/routes/index.js"] }, "q-c6f81dd2.js": { "size": 1346, "imports": ["q-2010d20b.js", "q-fccc4c26.js"], "origins": ["src/entry_RouterHead.js", "src/s_zrbrqoaqxsy.js"], "symbols": ["s_zrbrqoaqXSY"] }, "q-cb51c4f2.js": { "size": 2511, "imports": ["q-2010d20b.js", "q-fccc4c26.js"], "origins": ["src/entry_flower.js", "src/routes/flower/flower.css?used&inline", "src/s_dznigalrcag.js", "src/s_ow4nu0i1yz8.js", "src/s_p4uitwsjc7c.js", "src/s_v0y6u0vd1ey.js"], "symbols": ["s_dznIGAlrcag", "s_OW4nu0I1yZ8", "s_p4UiTwsJc7c", "s_V0Y6u0VD1eY"] }, "q-ccb17771.js": { "size": 397, "imports": ["q-2010d20b.js"], "dynamicImports": ["q-9ee91a9c.js"], "origins": ["src/components/header/header.js", "src/entry_layout.js", "src/s_vklnxphuh5s.js"], "symbols": ["s_VkLNXphUh5s"] }, "q-de034b6e.js": { "size": 185, "imports": ["q-2010d20b.js"], "dynamicImports": ["q-7fec964f.js"], "origins": ["src/components/weather/index.tsx", "src/entry_weather.js", "src/s_hoyxtywcypq.js"], "symbols": ["s_HoYXtYWcYPQ"] }, "q-e9d9d4cc.js": { "size": 53358, "imports": ["q-0b915655.js"], "origins": ["node_modules/@babel/runtime/helpers/esm/assertThisInitialized.js", "node_modules/@babel/runtime/helpers/esm/extends.js", "node_modules/@babel/runtime/helpers/esm/inheritsLoose.js", "node_modules/@babel/runtime/helpers/esm/objectWithoutPropertiesLoose.js", "node_modules/@babel/runtime/helpers/esm/setPrototypeOf.js", "node_modules/@emotion/is-prop-valid/dist/emotion-is-prop-valid.esm.js", "node_modules/@emotion/react/dist/emotion-react.browser.esm.js", "node_modules/@emotion/styled/base/dist/emotion-styled-base.browser.esm.js", "node_modules/@emotion/styled/dist/emotion-styled.browser.esm.js", "node_modules/@mui/material/Button/Button.js", "node_modules/@mui/material/Button/buttonClasses.js", "node_modules/@mui/material/ButtonBase/ButtonBase.js", "node_modules/@mui/material/ButtonBase/Ripple.js", "node_modules/@mui/material/ButtonBase/TouchRipple.js", "node_modules/@mui/material/ButtonBase/buttonBaseClasses.js", "node_modules/@mui/material/ButtonBase/touchRippleClasses.js", "node_modules/@mui/material/ButtonGroup/ButtonGroupContext.js", "node_modules/@mui/material/colors/blue.js", "node_modules/@mui/material/colors/common.js", "node_modules/@mui/material/colors/green.js", "node_modules/@mui/material/colors/grey.js", "node_modules/@mui/material/colors/lightBlue.js", "node_modules/@mui/material/colors/orange.js", "node_modules/@mui/material/colors/purple.js", "node_modules/@mui/material/colors/red.js", "node_modules/@mui/material/styles/createMixins.js", "node_modules/@mui/material/styles/createPalette.js", "node_modules/@mui/material/styles/createTheme.js", "node_modules/@mui/material/styles/createTransitions.js", "node_modules/@mui/material/styles/createTypography.js", "node_modules/@mui/material/styles/defaultTheme.js", "node_modules/@mui/material/styles/shadows.js", "node_modules/@mui/material/styles/styled.js", "node_modules/@mui/material/styles/useThemeProps.js", "node_modules/@mui/material/styles/zIndex.js", "node_modules/@mui/private-theming/useTheme/ThemeContext.js", "node_modules/@mui/private-theming/useTheme/useTheme.js", "node_modules/@mui/styled-engine/index.js", "node_modules/@mui/system/esm/borders.js", "node_modules/@mui/system/esm/breakpoints.js", "node_modules/@mui/system/esm/colorManipulator.js", "node_modules/@mui/system/esm/compose.js", "node_modules/@mui/system/esm/createStyled.js", "node_modules/@mui/system/esm/createTheme/createBreakpoints.js", "node_modules/@mui/system/esm/createTheme/createSpacing.js", "node_modules/@mui/system/esm/createTheme/createTheme.js", "node_modules/@mui/system/esm/createTheme/shape.js", "node_modules/@mui/system/esm/cssGrid.js", "node_modules/@mui/system/esm/display.js", "node_modules/@mui/system/esm/flexbox.js", "node_modules/@mui/system/esm/getThemeValue.js", "node_modules/@mui/system/esm/memoize.js", "node_modules/@mui/system/esm/merge.js", "node_modules/@mui/system/esm/palette.js", "node_modules/@mui/system/esm/positions.js", "node_modules/@mui/system/esm/propsToClassKey.js", "node_modules/@mui/system/esm/shadows.js", "node_modules/@mui/system/esm/sizing.js", "node_modules/@mui/system/esm/spacing.js", "node_modules/@mui/system/esm/style.js", "node_modules/@mui/system/esm/styleFunctionSx/styleFunctionSx.js", "node_modules/@mui/system/esm/typography.js", "node_modules/@mui/system/esm/useTheme.js", "node_modules/@mui/system/esm/useThemeProps/getThemeProps.js", "node_modules/@mui/system/esm/useThemeProps/useThemeProps.js", "node_modules/@mui/system/esm/useThemeWithoutDefault.js", "node_modules/@mui/utils/esm/ClassNameGenerator/ClassNameGenerator.js", "node_modules/@mui/utils/esm/capitalize.js", "node_modules/@mui/utils/esm/composeClasses/composeClasses.js", "node_modules/@mui/utils/esm/deepmerge.js", "node_modules/@mui/utils/esm/formatMuiErrorMessage.js", "node_modules/@mui/utils/esm/generateUtilityClass/generateUtilityClass.js", "node_modules/@mui/utils/esm/generateUtilityClasses/generateUtilityClasses.js", "node_modules/@mui/utils/esm/resolveProps.js", "node_modules/@mui/utils/esm/setRef.js", "node_modules/@mui/utils/esm/useEnhancedEffect.js", "node_modules/@mui/utils/esm/useEventCallback.js", "node_modules/@mui/utils/esm/useForkRef.js", "node_modules/@mui/utils/esm/useIsFocusVisible.js", "node_modules/clsx/dist/clsx.m.js", "node_modules/react-transition-group/esm/TransitionGroup.js", "node_modules/react-transition-group/esm/TransitionGroupContext.js", "node_modules/react-transition-group/esm/utils/ChildMapping.js", "src/entry_MUIButton.js", "src/s_r35ytgxhel0.js"], "symbols": ["s_r35YTGxHeL0"] }, "q-ed762435.js": { "size": 58, "imports": ["q-2010d20b.js"] }, "q-fccc4c26.js": { "size": 6324, "imports": ["q-2010d20b.js"], "dynamicImports": ["q-9949742d.js", "q-c6f81dd2.js"], "origins": ["../../node_modules/@builder.io/qwik-city/index.qwik.mjs", "@builder.io/qwik/build", "src/components/router-head/router-head.js", "src/entry_root.js", "src/s_3sccycdd1z0.js"], "symbols": ["s_3sccYCDd1Z0"] }, "q-fd23dcb3.js": { "size": 225, "imports": ["q-2010d20b.js"], "dynamicImports": ["q-0468e5e4.js"], "origins": ["src/routes/react/index.js"] } }, "injections": [{ "tag": "link", "location": "head", "attributes": { "rel": "stylesheet", "href": "/build/q-4157df4c.css" } }], "version": "1", "options": { "target": "client", "buildMode": "production", "forceFullBuild": true, "entryStrategy": { "type": "smart" } }, "platform": { "qwik": "0.9.0", "vite": "", "rollup": "2.70.1", "env": "node", "os": "darwin", "node": "16.14.0" } };
const RouterHead = /* @__PURE__ */ componentQrl(inlinedQrl(() => {
  ...
  return /* @__PURE__ */ jsx(Fragment, {
    children: [
      ...
      head2.meta.map((m) => /* @__PURE__ */ jsx("meta", __spreadValues({}, m))),
      head2.links.map((l) => /* @__PURE__ */ jsx("link", __spreadValues({}, l))),
      head2.styles.map((s) => /* @__PURE__ */ jsx("style", __spreadProps(__spreadValues({}, s.props), {
...
var global$1 = "";
var Root = /* @__PURE__ */ componentQrl(inlinedQrl(() => {
...
function render(opts) {
  return qwikReact.renderToStream(/* @__PURE__ */ jsx(Root, {}), __spreadValues({
    manifest
  }, opts));
}
...
})(["@builder.io/qwik", "@builder.io/qwik/jsx-runtime", "react", "react-dom", "@emotion/react"]);
qwikCityGenerate(render, {
  origin: "https://qwik.builder.io",
  outDir: path.join(Url.fileURLToPath(typeof document === "undefined" ? new (require("url")).URL("file:" + __filename).href : document.currentScript && document.currentScript.src || new URL("entry.static.js", document.baseURI).href), "..", "..", "dist")
});
