import {
  ...
  spread,
  style
} from "./chunk-HJRZ5VB7.js";
import {
  createComponent,
  createRenderEffect,
  mergeProps
} from "./chunk-QFEONFKE.js";
...
var findAttributes = new RegExp("(" + attrName + `\\s*=\\s*)(['"(]?)<!--#-->(['")]?)`, "gi");
...
function createHTML(r, {
  ...
  function createTemplate(statics) {
    ...
    const [html2, code] = parseTemplate(parse(markup)), templates = [];
    for (let i2 = 0; i2 < html2.length; i2++) {
      templates.push(document.createElement("template"));
      templates[i2].innerHTML = html2[i2];
  ...
  function parseKeyValue(tag, name, isSVG, isCE, options) {
    let count = options.counter++, expr = `!doNotWrap ? exprs[${count}]() : exprs[${count}]`, parts, namespace;
    ...
    if (name === "style") {
      ...
      if (ns)
        options.exprs.push(`r.setAttributeNS(${tag},"${ns}","${name}",${expr})`);
      else
...
${childOptions.exprs.join(";\n")};
  ...
  function processComponent(node, options) {
    ...
    for (let i = 0; i < keys.length; i++) {
      const name = keys[i], value = node.attrs[name];
      if (name === "###") {
    ...
    if (options.parent)
      options.exprs.push(`r.insert(${options.parent}, r.createComponent(exprs[${componentIdentifier}],${processComponentProps(propGroups)})${tag ? `, ${tag}` : ""})`);
    else
      options.exprs.push(`${options.fragment ? "" : "return "}r.createComponent(exprs[${componentIdentifier}],${processComponentProps(propGroups)})`);
    options.path = tag;
    options.first = false;
  }
...
`}}()`);
      ...
      if (options.multi) {
        ...
        options.exprs.push(`r.insert(${options.parent}, exprs[${options.counter++}])`);
      options.path = tag;
      options.first = false;
    }
  }
...
// dep:solid-js_html
var solid_js_html_default = index;
export {
  solid_js_html_default as default
};
