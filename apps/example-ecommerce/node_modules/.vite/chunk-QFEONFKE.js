...
function createResource(source, fetcher, options) {
  ...
  if (options.globalRefetch !== false) {
    Resources || (Resources = /* @__PURE__ */ new Set());
    Resources.add(load);
    Owner && onCleanup(() => Resources.delete(load));
  }
  const contexts = /* @__PURE__ */ new Set(), [s, set] = createSignal(options.initialValue), [track, trigger] = createSignal(void 0, {
    equals: false
  }), [loading, setLoading] = createSignal(false), [error, setError] = createSignal();
  let err = void 0, pr = null, initP = null, id = null, loadedUnderTransition = false, scheduled = false, dynamic = typeof source === "function";
  ...
  function read() {
    const c = SuspenseContext && lookup(Owner, SuspenseContext.id), v = s();
    if (err)
      throw err;
  ...
  function load(refetching = true) {
    ...
    scheduled = false;
    setError(err = void 0);
    const lookup2 = dynamic ? source() : source;
...
function SuspenseList(props) {
  let index = 0, suspenseSetter, showContent, showFallback;
  ...
  createComputed(() => {
    const reveal = props.revealOrder, tail = props.tail, visibleContent = showContent ? showContent() : true, visibleFallback = showFallback ? showFallback() : true, reverse = reveal === "backwards";
    if (reveal === "together") {
    ...
    for (let i = 0, len = registry.length; i < len; i++) {
      const n = reverse ? len - i - 1 : i, s = registry[n].inFallback();
      if (!stop && !s) {
...
//# sourceMappingURL=chunk-QFEONFKE.js.map
